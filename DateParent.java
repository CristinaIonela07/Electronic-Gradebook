...
public class DateParent {
    JFrame frame = new JFrame("Parent_Name");
    JButton next;
    JLabel lname;
    JTextField name;
    ...
    public DateParent() {
        ...
        ActionListener al = new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                String parent_name = name.getText();
                for (Course course : Catalog.getInstance().getCourses()) {
                    for (Group g : course.getGroups().values()) {
                        for (Student s : g) {
                            if (s.getMother().toString().equals(parent_name)) {
                                catalog.addObserver(s.getMother());
                                new ParentPage(s.getMother());
                                break;
                            }
                            else if(s.getFather().toString().equals(parent_name)){
                                catalog.addObserver(s.getFather());
                                new ParentPage(s.getFather());
                                break;
                            }
                        }
                    }
