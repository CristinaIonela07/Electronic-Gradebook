...
public class DateAssistant {
    JFrame frame = new JFrame("Assistant_Name");
    JButton next;
    JLabel lname;
    JTextField name;
    public DateAssistant(ScoreVisitor scoreVisitor) {
        ...
        ActionListener al = new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                String assistant_name = name.getText();
                for (Course course : Catalog.getInstance().getCourses()) {
                    for (Group g : course.getGroups().values()) {
                        if ((g.getAssistant().toString()).equals(assistant_name)) {
                            new AssistantPage(g.getAssistant(), scoreVisitor);
                            break;
                        }
                    }
